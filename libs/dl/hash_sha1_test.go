...
func Test__libdl_hash_sha1_WithHasherOverride(t *testing.T) {
	t.Parallel()
	...
	for _, shell := range libtest.SupportedShells() {
		for _, tcHashHandler := range []string{"auto", "sha1sum"} {
...
func Test__libdl_hash_sha1_WithAutoDetectHasher(t *testing.T) {
	t.Parallel()
	libtest.AssertShellsAvailable(t)
	...
	for _, shell := range libtest.SupportedShells() {
		for _, tc := range tests {
			...
			t.Run(fmt.Sprintf("%s-%s", shell, tc.name), func(t *testing.T) {
				cmd := exec.Command(shell, "-c", fmt.Sprintf(". ./dl.sh && . ../logger/logger.sh && __libdl_hash_sha1 %s", tc.file))
				libtest.DebugPrintCmd(t, cmd)
				var stdoutBuf, stderrBuf bytes.Buffer
				cmd.Stdout = &stdoutBuf
				cmd.Stderr = &stderrBuf
